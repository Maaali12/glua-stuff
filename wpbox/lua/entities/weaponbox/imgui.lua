...
function wpgui.IsDeveloperMode()
	return not wpgui.DisableDeveloperMode and devCvar:GetInt() > 0
end

local _devMode = false -- cached local variable updated once in a while

function wpgui.Hook(name, id, callback)
	local hookUniqifier = debug.getinfo(4).short_src
	hook.Add(name, "wpgui / " .. id .. " / " .. hookUniqifier, callback)
end
...
wpgui.Hook("PreRender", "Input", function()
...
hook.Add("NotifyShouldTransmit", "wpgui / ClearRenderBounds", function(ent, shouldTransmit)
	if shouldTransmit and ent._wpguiRBExpansion then
		ent._wpguiRBExpansion = nil
	end
end)
...
function wpgui.Start3D2D(pos, angles, scale, distanceHide, distanceFadeStart)
	...
	if gState.rendering == true then
		print(
			"[wpgui] Starting a new wpgui context when previous one is still rendering" ..
	...
	_devMode = wpgui.IsDeveloperMode()
...
function wpgui.Entity3D2D(ent, lpos, lang, scale, ...)
	gState.entity = ent
	local ret = wpgui.Start3D2D(ent:LocalToWorld(lpos), ent:LocalToWorldAngles(lang), scale, ...)
	if not ret then
		gState.entity = nil
	end
	return ret
...
function wpgui.ExpandRenderBoundsFromRect(x, y, w, h)
	local ent = gState.entity
	if IsValid(ent) then
		-- make sure we're not applying same expansion twice
		local expansion = ent._wpguiRBExpansion
		if expansion then
		...
		if _devMode then
			print("[wpgui] Updated renderbounds of ", ent, " to ", minrb, "x", maxrb)
		end

		ent._wpguiRBExpansion = {x, y, w, h}
	else
		if _devMode then
			print("[wpgui] Attempted to update renderbounds when entity is not valid!! ", debug.traceback())
		end
	end
end
...
local function drawDeveloperInfo()
	...
	developerText("wpgui developer", 0, 5, titleCol)
	...
	local ent = gState.entity
	if IsValid(ent) and ent._wpguiRBExpansion then
		local ex, ey, ew, eh = unpack(ent._wpguiRBExpansion)
...
function wpgui.CursorPos()
	local mx, my = gState.mx, gState.my
	return mx, my
end

function wpgui.IsHovering(x, y, w, h)
	local mx, my = gState.mx, gState.my
	return mx and my and mx >= x and mx <= (x + w) and my >= y and my <= (y + h)
end
function wpgui.IsPressing()
	return shouldAcceptInput() and gState.pressing
end
function wpgui.IsPressed()
...
-- Cached wpguiFontNamd->GModFontName
local _wpguiFontToGmodFont = {}

local EXCLAMATION_BYTE = string.byte("!")
function wpgui.xFont(font, defaultSize)
	-- special font
	if string.byte(font, 1) == EXCLAMATION_BYTE then

		local existingGFont = _wpguiFontToGmodFont[font]
		...
		local fontName = string.format("wpgui_%s_%d", name, size)
		_wpguiFontToGmodFont[font] = fontName
...
function wpgui.xButton(x, y, w, h, borderWidth, borderClr, hoverClr, pressColor)
	local bw = borderWidth or 1

	local bgColor = wpgui.IsHovering(x, y, w, h) and wpgui.skin.backgroundHover or wpgui.skin.background
	local borderColor =
		((wpgui.IsPressing() and wpgui.IsHovering(x, y, w, h)) and (pressColor or wpgui.skin.borderPress))
		or (wpgui.IsHovering(x, y, w, h) and (hoverClr or wpgui.skin.borderHover))
		or (borderClr or wpgui.skin.border)
	...
	return shouldAcceptInput() and wpgui.IsHovering(x, y, w, h) and gState.pressed
end

function wpgui.xCursor(x, y, w, h)
	local fgColor = wpgui.IsPressing() and wpgui.skin.foregroundPress or wpgui.skin.foreground
	local mx, my = gState.mx, gState.my
...
function wpgui.xTextButton(text, font, x, y, w, h, borderWidth, color, hoverClr, pressColor)
	local fgColor =
		((wpgui.IsPressing() and wpgui.IsHovering(x, y, w, h)) and (pressColor or wpgui.skin.foregroundPress))
		or (wpgui.IsHovering(x, y, w, h) and (hoverClr or wpgui.skin.foregroundHover))
		or (color or wpgui.skin.foreground)

	local clicked = wpgui.xButton(x, y, w, h, borderWidth, color, hoverClr, pressColor)

	font = wpgui.xFont(font, math.floor(h * 0.618))
